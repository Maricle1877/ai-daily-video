...
- `skill_dir`: WorkBuddy 本地 skill 目录（如 `~/.workbuddy/skills/ai-daily-video/`）
...
2. **自动截断**：summary 按句子边界截断到 45 字，highlight 按句子边界截断到 30 字（避免说话断一半）
3. 用 edge-tts 逐段生成 mp3 文件（临时目录）
4. 测量每段配音时长
5. 修改 HTML 的 `data-duration`（使场景停留 = 配音时长 + 1.8秒余量）
6. 用调整后的 HTML 重新录制视频（内部调用 Playwright）
7. 用 ffmpeg 将所有配音按时间轴混入视频音轨
8. 输出最终 MP4
